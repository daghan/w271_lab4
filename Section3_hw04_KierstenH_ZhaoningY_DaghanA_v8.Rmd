...
---
```{r global_options, warning=FALSE, message=FALSE, size="tiny", fig.height=3}
knitr::opts_chunk$set(cache=TRUE, tidy = TRUE, tidy.opts=list(width.cutoff=70))
options(width = 90) 
library(easypackages)
packages("knitr","xts","forecast","ggfortify","ggplot2", "dplyr","plotly", "Hmisc",
         "tseries","stats","fpp", "forcats", "gridExtra")
rm(list=ls())
```
# I. Introduction  
...
We proceeded by converting the data to a time object.
```{r size="tiny"}
...
```{r fig.height=3}
p1 <- ms.training %>% ggtsdisplay(main="Time Series Display of Unidentified Process")
```
...
plot(fit.stl, main = "STL Decomposition of the Time Series ")
```
...
ms.training.1d %>% ggtsdisplay(main="Exploring the 1st Order Differencing")
```
...
However, when we examined the time series plot as well as the PACF plot, we observed a strong yearly seasonal component (at lag 12). In order to examine the seasonal component more closely, we looked at the timeseries grouped by months. Upon inspection, it is clear that the mean of the time series varies over the year in a complex pattern. However, the variation is across months was stabilized by 12 month seasonal differencing. This suggests that the seasonality is yearly.  We did examine the effect of differencing for 4 month or 6 month seasonality (not shown for brevity) and it did not stabilize the mean across months. We are thus confident that there is yearly seasonality in this time series.

```{r fig.height=3}
p1 <- ms.training %>% ggmonthplot(main="Time Series Grouping without Lag")
p2 <- ms.training %>% diff(lag=12) %>% ggmonthplot(main="Time Series Grouping with Lag(12)")
grid.arrange(p1,p2, ncol = 2)
```

This yearly seasonal component warranted attention because it would most likely affect stationarity. Thus we proceeded to explore the effect of differencing for seasonality on our time series.

###Differencing for Yearly Seasonality  
```{r fig.height=3}
# Seasonal differencing only
ms.training.12d <- diff(ms.training, lag = 12)
ms.training.12d %>% ggtsdisplay(main = "Exploring the Yearly Seaonality")
```
...
ms.training.1d.12d %>% ggtsdisplay(main="Combining 1st Order and Yearly Differencing")
```
...
+ Our exploratory analysis of the time series suggests an ARIMA$(p,1,q)(P,1,D)_{12}$ model
+ Based on our interpretation of the ACF and PACF plots, our exploration to identify non-seasonal AR/MA order (p and q) considered order up to 5 and seasonal AR/MA order (P and Q) considered order up to 2
+ However, to avoid overfitting, the goal of our model estimation process was to identify approapriate models where the order of $p \in \{1,2\}, q \in \{1,2\}, P \in \{0,1\},\ and\ Q \in \{0,1\}$.

# IV. Identifying the Dependence Orders for Estimated Models
Based on the EDA, we will search p/q values from 0 to 5, that minimize various __Information Criteria__. We will make our final choice based on the mean absolute percentage error **(MAPE)** criterion. During our search, we will hold d=1, D=1, P=1, Q=1 constant (based on our EDA).

```{r}
results <- data.frame(p = 1:324, q = 1:324, P = 1:324, Q = 1:324, AIC = 999, AICc = 999, BIC = 999, MAPE = 100)
for (p in 0:5){
  for (q in 0:5){
    for (P in 0:2){
      for (Q in 0:2){
        tryCatch({
            m <- ms.training %>% Arima(order = c(p, 1, q), seasonal=list(order=c(P,1,Q),period=12))
            mape <- accuracy(m)[5]
            index <- (p*54) + (q*9) + (P*3) + (Q + 1)
            results[index,] = c(p,q,P,Q,m$aic, m$aicc, m$bic, mape)},
...
rbind(results[which.min(results$AIC),], results[which.min(results$AICc),], 
      results[which.min(results$BIC),], results[which.min(results$MAPE),])
```

## Candidate Models

Using the grid search (but also using our manual searches-not shown for brevity), we found an optimal p,q,P,D combination within the $p \in \{1,5\}, q \in \{1,5\}, P \in \{0,2\},\ and \  Q \in \{0,2\}$.
...
- ARIMA(2,1,1)(2,1,2)[12] because it minimized the AIC and AICc.

While ARIMA(4,1,5)(2,1,2)[12] is tempting because it minimizes in-sample MAPE performance, this is due to overfit. Forecast accuracy has shown that it has a very poor out-of-sample MAPE score (5.2, R code not shown here for brevity / page constraints).

#Model Parameter Estimation
```{r}
summary(fit1 <- ms.training %>% Arima(order = c(1, 1, 1), seasonal=list(order=c(0,1,1),period=12)))
summary(fit2 <- ms.training %>% Arima(order = c(2, 1, 1), seasonal=list(order=c(2,1,1),period=12)))
```
...
We wanted to have some kind of "standard" for model performance, in order to evaluate the performance of our model accuracy. If we had information about the domain of the data generating process, that might have provided a guideline on what kind of model accuracy might be expected and acceptable.
...
```{r}
forecast1 <- fit1 %>% forecast(h=11)
forecast2 <- fit2 %>% forecast(h=11)
(results <- rbind( accuracy(forecast1,ms.test), accuracy(forecast2,ms.test))[,1:5])
```

We compared our forecast fit to our reserved test data using the MAPE score.

We note that the $ARIMA(1,1,1)(0,1,1)_{12}$ has not only a lower score than the $ARIMA(2,1,1)(2,1,1)_{12}$, (3.2 versus 4%), but also the lowest out of sample MAPE score for any of the models we estimated (full search not shown here for breviety). We therefore  performed forecasting with the $ARIMA(1,1,1)(0,1,1)_{12}$ model.
...
autoplot(forecast1) + autolayer(ms.test, series = "Actual") + autolayer(forecast1$mean, series="Forecasts")
```
